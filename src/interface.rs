...
pub fn gateway() {
    ...
    match &cli.command {
        ...
        Commands::Done { index } => {
            ...
            match todo.done(idx) {
                Ok(_) => {
                    println!("{}", "Todo checked off!".bright_green().bold())
                }
                Err(_) => {
        ...
        Commands::Remove { index } => {
            ...
            match todo.delete(idx) {
                Ok(_) => println!("{}", "Todo deleted!".bright_red().bold()),
                Err(_) => {
        ...
        Commands::Change { index, new } => {
            ...
            match todo.modify(idx, &new) {
                Ok(_) => {
                    println!("{}", "Todo modified!".bright_green().bold())
                }
                Err(_) => {
        ...
        Commands::Clear => match todo.drop() {
            Ok(_) => println!("{}", "Cleared all todos!".bright_red().bold()),
            Err(_) => {
